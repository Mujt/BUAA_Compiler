...
/*
char reg[][4]={"$t0","$t1","$t2","$t3","$t4","$sp","$fp","$ra"};
char opcod[][5]={"add","sub","mult","div","mfhi","mflo","addi","lw","lb","sw","sb",
"syscall","li","la","move","j","jal","jr","beq","bne","bgez","bgtz","blez","bltz"};
void write_text(int opc,int r1,int r2,int r3,char s[],int num){
    if(opc==0)fputs(s,out);
    if(opc>=1 && opc <=4){
        fputs(opcod[opc-1],out);
        fprintf(out," ");
        fputs(reg[r1],out);

    }
}*/
void write_file_data(char st1[],char st2[],char st3[]){
    fputs(st1,out);
    fputs("   ",out);
    fputs(st2,out);
    fputs("   ",out);
...
void data(){
    int i=0;
    int arysize;
    char arysizestr[10],charvl[4],strvl[105],numarypre[50];
        ...
        if(midsy == consts){
            ...
            if(midsy == intsign){
                if(strlen(midtab.Midcod[i].src2)==0){
                    /*strcpy(numarypre,midtab.Midcod[i].res);
                    strcat(numarypre,"kk");
                    emitdata(numarypre,3,"0");*/
                ...
                if(strlen(midtab.Midcod[i].src2)==0){
                    emitdata(midtab.Midcod[i].res,1,"0");
                }
                else {
    ...
    for(i=0;i<DATB.dataptr;i++){
        ...
        write_file_data(DATB.dataone[i].dataname,dattype[DATB.dataone[i].datatyp-1],DATB.dataone[i].datavalue);
    }
    //printf("printf data!\n");
    printf("enter:   .asciiz   %c%c%c%c\n",'\"','\\','n','\"');
    char stt[5]={'"','\\','n','"'};
    write_file_data("enter:",".asciiz",stt);
}
...
void stak_sub(int des){
    ...
    int i=MTable.Tab[des].paranum,j=0;
    while(i>0){
        printf("lw $t0,%d($fp)\n",j*4+MTable.Tab[des].spsize+4);
        printf("sw $t0,%d($fp)\n",MTable.Tab[des].spsize-4-i*4);
        i--;
        j++;
    }
}
...
void ret(int k,int des){
    ...
    if(strcmp(midtab.Midcod[k].res,"\0")!=0){
        ...
        printf("addi $sp,$sp,%d\n",MTable.Tab[des].spsize+4);
        printf("jr $ra\n");
        if(midtab.Midcod[k+1].midop == endfsign)
        returnsy=1;
        return;
}
...
void deal_asseq(int k,int des){
    ...
    if(var2==0){
            if(isC(midtab.Midcod[k].src1)==1){
                ...
                var2=find_data(midtab.Midcod[k].src1);
...
int deal_state(int ik,int des){
    ...
    case jnesign:
        ...
        if(var_des==0){
            if(isC(midtab.Midcod[ik].src1)==1){
                printf("li $t0,%s\n",midtab.Midcod[ik].src1);
                strcpy(varstr,"$t0");
            }
            else if(isNum(midtab.Midcod[ik].src1,&varnum)==0){
                printf("li $t0,%s\n",midtab.Midcod[ik].src1);
                strcpy(varstr,"$t0");
        }
            else {
                if(midtab.Midcod[ik].src1[0]!='\0'){
                var_des=find_data(midtab.Midcod[ik].src1);
                if(MTable.Tab[var_des].typ==0)
                printf("lw $t0,%s\n",midtab.Midcod[ik].src1);
                else
                     printf("lb $t0,%s\n",midtab.Midcod[ik].src1);
    ...
    case callsign:
        ...
        fun_des=loc(midtab.Midcod[ik].src1);
        if(MTable.Tab[fun_des].paranum!=0)
            printf("addi $sp,$sp,%d\n",MTable.Tab[fun_des].paranum*4);
        ik++;
        break;
    case retsign:
        ret(ik,des);
